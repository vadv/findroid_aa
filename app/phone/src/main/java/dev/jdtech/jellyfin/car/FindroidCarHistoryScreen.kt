...
class FindroidCarHistoryScreen(
    ...
    override fun onGetTemplate(): Template {
        if (loading && items.isEmpty()) {
            return MessageTemplate.Builder("Загружаю историю…")
        ...
        statusText?.let { list.addItem(Row.Builder().setTitle(it).build()) }
    ...
    private fun loadHistory() {
        scope.launch {
            ...
            val status =
                FindroidCarNetworkRetry.attempt("History") {
                    withContext(Dispatchers.IO) {
                        ...
                        val onlineResumeItems = jellyfinRepository.getResumeItems()
