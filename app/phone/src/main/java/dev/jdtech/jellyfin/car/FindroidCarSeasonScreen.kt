...
class FindroidCarSeasonScreen(
    ...
    override fun onGetTemplate(): Template {
        if (loading && episodes.isEmpty()) {
            return MessageTemplate.Builder("Загружаю эпизоды…")
        ...
        val statusMessage =
            when {
                ...
                episodes.isEmpty() -> "No episodes returned for this season"
                else -> null
            }
        if (statusMessage != null) {
            listBuilder.addItem(statusRow(statusMessage))
        }
        if (episodes.isEmpty() && statusMessage != null) {
            // No content to show alongside the status row; render only the back + status rows.
        } else {
        ...
        scope.launch {
            ...
            when (status) {
                ...
                is FindroidCarNetworkRetry.Status.Failure -> {
                    Timber.w(
                        "FindroidCarSeasonScreen episodes load failed seriesId=%s seasonId=%s errorType=%s attempts=%s",
                        series.itemId,
                        season.itemId,
                        status.failure.javaClass.simpleName,
                        status.attempts,
                    )
                    errorMessage =
