...
class FindroidCarVideoScreen(
    ...
    override fun onGetTemplate(): Template {
        ...
        return navigationTemplate(transparentActionStrip())
    }

    private fun transparentActionStrip(): ActionStrip =
        ActionStrip.Builder()
            .addAction(
                Action.Builder()
    ...
    override fun onClick(x: Float, y: Float) {
        ...
        if (controlsVisible) {
            if (handleBackClick(x, y)) return
            if (handleZoomClick(x, y)) return
            if (handleCenterClusterClick(x, y)) return
    ...
    private fun startPlayer(surfaceContainer: SurfaceContainer) {
        ...
        presentation.setTitle(item.displayTitleForPlayer())
        presentation.setIsPlaying(true)
        presentation.show()
    ...
    private fun pausePlayback() {
        ...
        exoPlayer.pause()
        isPlaying = false
        presentation?.setIsPlaying(false)
    ...
    private fun showControlsTemporarily() {
        ...
        val livePlaying = player?.isPlaying ?: isPlaying
        isPlaying = livePlaying
        presentation?.setIsPlaying(livePlaying)
    ...
    private fun handleBackClick(x: Float, y: Float): Boolean {
        if (x !in CONTROL_BACK_LEFT..(CONTROL_BACK_LEFT + CONTROL_BACK_SIZE)) return false
        if (y !in CONTROL_BACK_TOP..(CONTROL_BACK_TOP + CONTROL_BACK_SIZE)) return false
        Timber.i("FindroidCarVideoScreen back hit x=%s y=%s", x, y)
        goBack()
        return true
    }

    private fun handleZoomClick(x: Float, y: Float): Boolean {
        val right =
            visibleArea?.right?.toFloat()
                ?: surfaceContainer?.width?.toFloat()
                ?: return false
        val zoomLeft = right - CONTROL_ZOOM_RIGHT_OFFSET - CONTROL_BACK_SIZE
        if (x !in zoomLeft..(zoomLeft + CONTROL_BACK_SIZE)) return false
        if (y !in CONTROL_BACK_TOP..(CONTROL_BACK_TOP + CONTROL_BACK_SIZE)) return false
        aspectMode = aspectMode.next()
        val appliedMode = presentation?.setAspectMode(aspectMode) ?: "UNKNOWN"
        Timber.i(
            "FindroidCarVideoScreen aspect mode tap requestedMode=%s appliedMode=%s x=%s y=%s",
            aspectMode.name,
            appliedMode,
            x,
            y,
    ...
    private fun handleCenterClusterClick(x: Float, y: Float): Boolean {
        val right =
            visibleArea?.right?.toFloat()
                ?: surfaceContainer?.width?.toFloat()
                ?: return false
        ...
        val cx = right / 2f
        val cy = bottom / 2f
        if (kotlin.math.abs(y - cy) > CENTER_HIT_VERTICAL_RADIUS) return false

        val playDistance = kotlin.math.abs(x - cx)
        if (playDistance <= PLAY_HIT_RADIUS) {
            Timber.i("FindroidCarVideoScreen play/pause hit x=%s y=%s", x, y)
        ...
        val centersSorted =
            listOf(
                "skipMinus30" to cx - 320f,
                "skipMinus10" to cx - 160f,
                "skipPlus10" to cx + 160f,
                "skipPlus30" to cx + 320f,
            )
        val nearest = centersSorted.minBy { kotlin.math.abs(x - it.second) }
        val distance = kotlin.math.abs(x - nearest.second)
        if (distance > SKIP_HIT_RADIUS) return false
        Timber.i("FindroidCarVideoScreen skip hit action=%s x=%s y=%s", nearest.first, x, y)
        when (nearest.first) {
            "skipMinus30" -> seekBy(-SEEK_LONG_MS)
            "skipMinus10" -> goToAdjacentEpisode(-1)
            "skipPlus10" -> goToAdjacentEpisode(+1)
            "skipPlus30" -> seekBy(SEEK_LONG_MS)
        }
        return true
    }

    private fun goToAdjacentEpisode(direction: Int) {
        val seriesIdStr = item.seriesId ?: return
        val seasonIdStr = item.seasonId ?: return
        val currentIndex = item.indexNumber ?: return
        val targetIndex = currentIndex + direction
        Timber.i(
            "FindroidCarVideoScreen goToAdjacentEpisode dir=%s seriesId=%s seasonId=%s currentIndex=%s targetIndex=%s",
            direction,
            seriesIdStr,
            seasonIdStr,
            currentIndex,
        ...
        scope.launch {
            val episodesStatus =
                FindroidCarNetworkRetry.attempt(
                    "VideoScreen:adjacent:${seriesIdStr}:${seasonIdStr}"
                ) {
                    withContext(Dispatchers.IO) {
                        jellyfinRepository.getEpisodes(
                            java.util.UUID.fromString(seriesIdStr),
                            java.util.UUID.fromString(seasonIdStr),
                        )
                    }
                }
            val episodes =
                when (episodesStatus) {
                    is FindroidCarNetworkRetry.Status.Success -> episodesStatus.value
                    is FindroidCarNetworkRetry.Status.Failure -> {
                        Timber.w(
                            episodesStatus.failure,
                            "FindroidCarVideoScreen goToAdjacentEpisode getEpisodes failed after %d attempts",
                            episodesStatus.attempts,
                        )
                        return@launch
                    }
                }
            val target = episodes.firstOrNull { it.indexNumber == targetIndex } ?: return@launch
            val carItem = target.toFindroidCarCatalogItem(carContext.filesDir) ?: return@launch
            Timber.i(
                "FindroidCarVideoScreen switching to episode indexNumber=%s itemId=%s",
                target.indexNumber,
                carItem.itemId,
            )
    ...
    private fun handleSeekbarClick(x: Float, y: Float): Boolean {
        ...
        val bottom =
            visible?.bottom?.toFloat()
                ...
                ?: return false
        val centerY =
            if (controlsVisible) bottom - SEEK_BAR_CENTER_OFFSET_FROM_VISIBLE_BOTTOM
            else bottom - SEEK_BAR_CLEAN_OFFSET_FROM_VISIBLE_BOTTOM
        val radius =
            if (controlsVisible) SEEK_BAR_HIT_VERTICAL_RADIUS
            else SEEK_BAR_CLEAN_HIT_VERTICAL_RADIUS
        if (y !in centerY - radius..centerY + radius) {
            return false
        }
        val trackLeft = if (controlsVisible) seekbarTrackLeft() else 0f
        val trackRight =
            if (controlsVisible) seekbarTrackRight()
            else (visible?.right?.toFloat() ?: surfaceContainer?.width?.toFloat() ?: 0f)
    ...
    private fun playerListener(): Player.Listener =
        object : Player.Listener {
            ...
            override fun onIsPlayingChanged(playing: Boolean) {
                isPlaying = playing
                presentation?.setIsPlaying(playing)
            }
    ...
    private companion object {
        const val CONTROLS_AUTO_HIDE_DELAY_MS = 5_000L
        ...
        const val CONTROL_BACK_LEFT = 24f
        const val CONTROL_BACK_TOP = 24f
        const val CONTROL_BACK_SIZE = 80f
        const val CONTROL_ZOOM_RIGHT_OFFSET = 128f
        const val PLAY_HIT_RADIUS = 64f
        const val SKIP_HIT_RADIUS = 60f
        const val CENTER_HIT_VERTICAL_RADIUS = 80f
        const val SEEK_BAR_HORIZONTAL_INSET = 96f
        const val SEEK_BAR_CENTER_OFFSET_FROM_VISIBLE_BOTTOM = 56f
        const val SEEK_BAR_HIT_VERTICAL_RADIUS = 44f
        const val SEEK_BAR_CLEAN_OFFSET_FROM_VISIBLE_BOTTOM = 2f
        const val SEEK_BAR_CLEAN_HIT_VERTICAL_RADIUS = 36f
