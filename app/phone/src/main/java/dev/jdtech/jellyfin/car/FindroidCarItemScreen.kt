...
class FindroidCarItemScreen(
    ...
    override fun onGetTemplate(): Template {
        if (loadingPlayback) {
            return MessageTemplate.Builder("Готовлю поток…")
                .setTitle(item.title)
    ...
    private fun resolveOnlineStreamAndPlay() {
        ...
        scope.launch {
            val status =
                FindroidCarNetworkRetry.attempt("Item:${item.itemId}:resolveSource") {
            ...
            when (status) {
                is FindroidCarNetworkRetry.Status.Success -> {
                    val source = status.value
                    loadingPlayback = false
                    pushVideo(
                        item.copy(
                            videoPath =
                                source.path.takeIf { source.type == FindroidSourceType.LOCAL },
                            streamUrl =
                                source.path.takeIf { source.type != FindroidSourceType.LOCAL },
                        )
                    )
                ...
                is FindroidCarNetworkRetry.Status.Failure -> {
                    loadingPlayback = false
                    playbackError = FindroidCarNetworkRetry.friendlyMessage(status.failure)
                    invalidate()
                }
            }
        }
