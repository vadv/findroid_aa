...
internal fun FindroidCarCatalogItem.displayTitleForPlayer(): String =
    when (itemKind) {
        FindroidCarCatalogItemKind.EPISODE -> {
            val season = parentIndexNumber?.let { "S$it" }
            val episode = indexNumber?.let { "E$it" }
            val prefix = listOfNotNull(season, episode).joinToString(":").takeIf { it.isNotBlank() }
            val raw = title.removePrefix(indexNumber?.let { "E${it.toString().padStart(2, '0')} - " } ?: "")
            listOfNotNull(prefix, raw.ifBlank { title }).joinToString(" — ")
        }
        else -> title
    }
