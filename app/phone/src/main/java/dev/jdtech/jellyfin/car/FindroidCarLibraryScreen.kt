...
class FindroidCarLibraryScreen(
    ...
    override fun onGetTemplate(): Template {
        if (loading && items.isEmpty()) {
            return MessageTemplate.Builder("Загружаю библиотеку…")
    ...
    private fun loadItems() {
        scope.launch {
            ...
            val status =
                FindroidCarNetworkRetry.attempt("Library:${source.name}:$activeTabId") {
            ...
            when (status) {
                is FindroidCarNetworkRetry.Status.Success -> {
                    items = status.value.sortedWith(carItemComparator)
                    errorMessage = null
                }
                is FindroidCarNetworkRetry.Status.Failure -> {
                    errorMessage =
                        if (items.isNotEmpty()) {
                            ...
                            mapTerminalErrorMessage(status.failure)
                        }
                }
            }
            loading = false
    ...
    private fun mapTerminalErrorMessage(throwable: Throwable): String =
        when {
            ...
            else -> FindroidCarNetworkRetry.friendlyMessage(throwable)
        }
